...
if __name__ == '__main__':
	...
	for i in range(len(data)):
		...
		data = pd.DataFrame([fcrs,fcon,fdir,f_r])
		data = data.T
		data.to_csv('Tamura_web_steam43_augmented.csv', mode='a', header=False)
		# data.to_csv('Tamura_steam43_augmented.csv', mode='a', header=False)
	...
	for m in text:
		for i in range(len(data)):
			...
			if img.mode == 'P':  # 必须是RGB模式 P是GIF的格式
				img = img.convert('RGB')
				img.save(
					'D:/Desktop/personality/personality-prediction-master/personality-prediction-master/data/personality_steam/web/web' + m + str(i + 1) + '.jpg')
			...
			data = pd.DataFrame([fcrs, fcon, fdir, f_r])
			data = data.T
			data.to_csv('Tamura_web_steam_augmented.csv', mode='a', header=False)
			# data.to_csv('Tamura_steam_augmented.csv', mode='a', header=False)
